<p align="center"><img src="https://i.imgur.com/A6bWGFl.gif"/></p>
<img align='right' src="https://media.giphy.com/media/M9gbBd9nbDrOTu1Mqx/giphy.gif" width="250">

---
</br>

### I'm Valerian Ahmad F! <img src="https://media.giphy.com/media/12oufCB0MyZ1Go/giphy.gif" width="50"></h2>

**Software Engineer & Full Stack Developer**  
*Passionate about building scalable services, modern frontend architectures, and thoughtful UI/UX.*

---
</br>

### <img src="https://media.giphy.com/media/WUlplcMpOCEmTGBtBW/giphy.gif" width="30"> Tech Stack
| Category | Technologies |
| :--- | :--- |
| **Languages** | TypeScript, JavaScript, Python, PHP, Rust, Go, Java, Kotlin, C, C++ |
| **Frontend** | React, Next.js, Vue.js, Angular, Tailwind CSS, Flutter, HTML5, CSS3 |
| **Backend** | Node.js, Express, NestJS, Laravel, Django, FastAPI, Flask |
| **Database** | PostgreSQL, MySQL, MongoDB, SQLite, Oracle |
| **Infrastructure** | Docker, Git, Linux, Cloudflare, Vercel |
| **Design** | Figma, UI/UX Principles |

---
<br/>

### 🚀 Focus Area

```typescript
const valerian = {
  expertise: ["Full Stack Engineering", "System Design"],
  interest:  ["Cloud Architecture", "UI/UX Engineering"],
  status:    "Open to collaboration & opportunities"
};

```

---
<br/>

### <img src="https://media.giphy.com/media/VgCDAzcKvsR6OM0uWg/giphy.gif" width="50"> Stats
<div align="center">
<img src="https://github-readme-stats.vercel.app/api?username=ValerianAhmad&show_icons=true&theme=graywhite&hide_border=true&include_all_commits=true&count_private=true&hide_title=true" height="150"/>
<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ValerianAhmad&theme=graywhite&hide_border=true&layout=compact&hide_title=true" height="180"/>
</div>

---
<br/>

### <img src="https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExN24wYnplbjd2azB5N2xnOTAybjVzdTVoaWZ4NzB4YWR1cmwzZ2I3bSZlcD12MV9naWZzX3NlYXJjaCZjdD1n/kH6CqYiquZawmU1HI6/giphy.gif" width="50"> Github Repos

[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=anandmainali&repo=PackageTemplate&show_owner=true)](https://github.com/ValerianAhmad/sugi-ac-mobil)
[![ReadMe Card](https://github-readme-stats.vercel.app/api/pin/?username=anandmainali&repo=Foods-Ecommerce&show_owner=true)](https://github.com/ValerianAhmad/c-minor.github.io)

---
<br/>

### <img src="https://media.giphy.com/media/LnQjpWaON8nhr21vNW/giphy.gif" width="50"> Connect

[Instagram](https://instagram.com/valeriaanaf) · [LinkedIn](https://linkedin.com/in/valerian-ahmad) · [TikTok](https://tiktok.com/@valerian_ahmd) · [Email](mailto:valerianahmdahmad@gmail.com)

---
<br/>

<div align="center">
  <sub>📍 Yogyakarta, Indonesia &nbsp;•&nbsp; © 2026 <a href="https://instagram.com/valeriaanaf">valeriaanaf</a>. Built with passion.</sub>
</div>
